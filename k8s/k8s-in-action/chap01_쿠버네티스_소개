...
- 여기서 시스템의 증가하는 부하를 처리할수 있는 방법 Scale up과 Scale out
...
- 개발팀이 애플리케이션을 배포하고 관리하는 것이 모두가 낫다고 생각한다. 
  - 사용자의 요구사항 및 바뀌어야할 문제점 등을 인지할 수 있음  
...
또한 도커나 rkt와 같은 컨테이너 기술이 어떤 문제를 해결하는지 이해해야 한다.

### 1.2.1 컨테이너 이해
- 애플리케이션이 더 작은 수의 커다란 구성요소로만 이뤄진 경우 구성요소에 전용 가상머신을 제공하고 고유한 운영체제 인스턴스를를 제공해 환경을 격리할수 있다.
- 하지만 MSA로 인해 구성요소가 작아지면서 숫자가 많아지기 시작하면 HW 리소스 낭비가 생길 수 있음.
...
- 가상머신
  ...
  - 호스트에 가상머신을 실행하면 완전히 분리된 운영체제가 실행되고 동일한 하드웨어를 공유.
...
- 첫번째는 리눅스 네임스페이스로 각 프로세스가 시스템(파일, 프로세스, 네트워크 인터페이스, 호스트 이름 등)에 대한 독립된 뷰만 볼 수 있도록 하는것
...
- 2개의 서로 다른 UTS 네임스페이스를 프로세스에 각각 지정하면 서로 다른 로컬 호스트 이름을 보게할 수 도 있음.(두 프로세스는 마치 두 개의 다른 시스템에서 실행중인 것처럼 보이게 할 수 있음.)
- 각 컨테이너는 고유한 네트워크 네임스페이슬르 사용하므로 각 컨테이너는 고유한 네트워크 인터페이스 세트를 볼 수 있음.
...
- 앱, 라이브러리, 여러 종속성, 심지어 전체 OS, FS까지도 도커를 실행하는 다른 컴퓨터에 애플리케이션을 프로비저닝하는 데 사용할 수 있다.
- 도커로 패키징된 앱을 실행하면 함께 제공된 파일 시스템 내용을 정확하게 볼수 있다.
- 앱은 실행 중인 서버의 내용은 볼 수 없으므로 서버에 개발 컴퓨터와 다른 설치 라이브러리가 설치돼 있는지는 중요하지 않다.
...
- 동일한 레이어르 포함하는 다른 컨테이너 이미지를 실행할 때 다른 레이어에서 이미 다운로드된 경우 이미지의 특정 레이어만 다운로드 하면됨.
...
- 파일을 공유하더라도 여전히 서로 격리돼 있는데 이것은 컨테이너 이미지 레이어가 읽기 전용이어서 컨테이너가 실행될때 이미지 레이어 위에 새로운 쓰기 가능한 레이어가 만들어진다.
...
- 머신이 다른 버전의 리눅스 커널로 실행되거나 동일한 커널 모듈을 사용할 수 없는 경우에 앱이 실행될 수 없다.
...
- k8s가 도커 기반 컨테이너만을 위해 만들어진 컨테이너 오케스트레이션 시스템이라고 생각해선 X 
...
컨트롤 플레인(Control Plane)
 ...
 - API서버:  사용자, 컨트롤 플레인 구성 요소와 통신
 - 스케줄러: 앱의 배포를 담당(앱의 배포 가능한 각 구성요소를 워크 노드에 할당)
 - 컨트롤러 매니저: 구성요소 복제본, 워커 노드 추적, 노드 장애 처리 등과 같은 클러스터단의 기능을 수행
 - Etcd: 클러스터 구성을 지속적으로 저장하는 신뢰할 수 있는 분산 데이터 저장소(kvs)
...
- API 서버가 앱 디스크립션을 처리할때 스케줄러는 각 컨테이너에 필요한 리소스를 계산하고 각 노드에 할당되지 않은 리소스를 기반으로 사용 가능한 워커 노드에 지정된 컨테이너를 할당한다.
- kebulet은 컨테이너 런타임(도커)에 필요한 컨테이너 이미지를 가져와 컨테이너를 실행하도록 지시한다.
...
복제본 수 스케일링
- 행되는 동안 복제본 수를 늘릴지 줄일지 결정할 수 있음.
...
- 특정 앱이 특정 종류의 하드웨어에서 실행해야 하는 경우에도 지원이 가능

하드웨어 활용도 높이기
- 인프라와 애플리케이션을 분리해서 생각할 수 있다.
...
상태 확인과 자가 치유 (선언적 구성)
- 서버 장애 발생시 언제든지 클러스터 간에 앱을 이동시킬 수 있는 시스템이 갖출수 있다.
...
- 서비스 디스커버리와 같은 일반적으로 구현해야 하는 기능들을 구현할 필요가 없어졌다.
